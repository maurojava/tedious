...
module.exports = class WritableTrackingBuffer {
  ...
  writeString(value: string, encoding: ?Encoding) {
    ...
    const bytesWritten = this.buffer.write(value, this.position, this.buffer.length - this.position, encoding);
  ...
  writeUsVarbyte(value: any, encoding: ?Encoding) {
    ...
    if (value instanceof Buffer) {
      ...
      this.makeRoomFor(length);
      this.buffer.write(value, this.position, this.buffer.length - this.position, encoding);
      return this.position += length;
    }
  }
